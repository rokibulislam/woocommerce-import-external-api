...
class Manufacture {
	...
	public function __construct() {
		...
		// Add form.
		add_action( 'product_manufacture_add_form_fields', array( $this, 'add_category_fields' ) );
		add_action( 'product_manufacture_edit_form_fields', array( $this, 'edit_category_fields' ), 10 );
		add_action( 'created_term', array( $this, 'save_category_fields' ), 10, 3 );
		add_action( 'edit_term', array( $this, 'save_category_fields' ), 10, 3 );

		// Add columns.
		add_filter( 'manage_edit-product_manufacture_columns', array( $this, 'product_manufacture_columns' ) );
		add_filter( 'manage_product_manufacture_custom_column', array( $this, 'product_manufacture_column' ), 10, 3 );

		// Add row actions.
		add_filter( 'product_manufacture_row_actions', array( $this, 'product_manufacture_row_actions' ), 10, 2 );
		add_filter( 'admin_init', array( $this, 'handle_product_manufacture_row_actions' ) );

		// Taxonomy page descriptions.
		add_action( 'product_manufacture_pre_add_form', array( $this, 'product_manufacture_description' ) );
		add_action( 'after-product_manufacture-table', array( $this, 'product_manufacture_notes' ) );
		...
		add_action( 'admin_footer', array( $this, 'scripts_at_product_manufacture_screen_footer' ) );
	...
	public function create_term( $term_id, $tt_id = '', $taxonomy = '' ) {
		if ( 'product_manufacture' != $taxonomy && ! taxonomy_is_product_attribute( $taxonomy ) ) {
			return;
		}
	...
	public function add_category_fields() {
		...
		<div class="form-field term-thumbnail-wrap">
			...
			<div id="product_manufacture_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( wc_placeholder_img_src() ); ?>" width="60px" height="60px" /></div>
			<div style="line-height: 60px;">
				<input type="hidden" id="product_manufacture_thumbnail_id" name="product_manufacture_thumbnail_id" />
			...
			<script type="text/javascript">
				...
				if ( ! jQuery( '#product_manufacture_thumbnail_id' ).val() ) {
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame.on( 'select', function() {
						...
						jQuery( '#product_manufacture_thumbnail_id' ).val( attachment.id );
						jQuery( '#product_manufacture_thumbnail' ).find( 'img' ).attr( 'src', attachment_thumbnail.url );
				...
				jQuery( document ).on( 'click', '.remove_image_button', function() {
					jQuery( '#product_manufacture_thumbnail' ).find( 'img' ).attr( 'src', '<?php echo esc_js( wc_placeholder_img_src() ); ?>' );
					jQuery( '#product_manufacture_thumbnail_id' ).val( '' );
					jQuery( '.remove_image_button' ).hide();
					return false;
				});
	...
	public function edit_category_fields( $term ) {
		...
		<tr class="form-field term-thumbnail-wrap">
			...
			<td>
				<div id="product_manufacture_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $image ); ?>" width="60px" height="60px" /></div>
				<div style="line-height: 60px;">
					<input type="hidden" id="product_manufacture_thumbnail_id" name="product_manufacture_thumbnail_id" value="<?php echo esc_attr( $thumbnail_id ); ?>" />
					<button type="button" class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'woocommerce' ); ?></button>
					<button type="button" class="remove_image_button button"><?php esc_html_e( 'Remove image', 'woocommerce' ); ?></button>
				</div>
				<script type="text/javascript">
					...
					// Only show the "remove image" button when needed
					if ( '0' === jQuery( '#product_manufacture_thumbnail_id' ).val() ) {
						jQuery( '.remove_image_button' ).hide();
					}
	...
	public function save_category_fields( $term_id, $tt_id = '', $taxonomy = '' ) {
		if ( isset( $_POST['display_type'] ) && 'product_manufacture' === $taxonomy ) { // WPCS: CSRF ok, input var ok.
			update_term_meta( $term_id, 'display_type', esc_attr( $_POST['display_type'] ) ); // WPCS: CSRF ok, sanitization ok, input var ok.
		}
		if ( isset( $_POST['product_manufacture_thumbnail_id'] ) && 'product_manufacture' === $taxonomy ) { // WPCS: CSRF ok, input var ok.
			update_term_meta( $term_id, 'thumbnail_id', absint( $_POST['product_manufacture_thumbnail_id'] ) ); // WPCS: CSRF ok, input var ok.
		}
	}

	/**
	 * Description for product_manufacture page to aid users.
	 */
	public function product_manufacture_description() {
		echo wp_kses(
	 ...
	 */
	public function product_manufacture_notes() {
		$category_id   = get_option( 'default_product_manufacture', 0 );
		$category      = get_term( $category_id, 'product_manufacture' );
	 ...
	 * @return array
	 */
	public function product_manufacture_columns( $columns ) {
		$new_columns = array();
	 ...
	 * @return array
	 */
	public function product_manufacture_row_actions( $actions, $term ) {
		$default_category_id = absint( get_option( 'default_product_manufacture', 0 ) );

		if ( $default_category_id !== $term->term_id && current_user_can( 'edit_term', $term->term_id ) ) {
			$actions['make_default'] = sprintf(
				'<a href="%s" aria-label="%s">%s</a>',
				wp_nonce_url( 'edit-tags.php?action=make_default&amp;taxonomy=product_manufacture&amp;post_type=product&amp;tag_ID=' . absint( $term->term_id ), 'make_default_' . absint( $term->term_id ) ),
	 ...
	 */
	public function handle_product_manufacture_row_actions() {
		if ( isset( $_GET['action'], $_GET['tag_ID'], $_GET['_wpnonce'] ) && 'make_default' === $_GET['action'] ) { // WPCS: CSRF ok, input var ok.
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], 'make_default_' . $make_default_id ) && current_user_can( 'edit_term', $make_default_id ) ) { // WPCS: Sanitization ok, input var ok, CSRF ok.
				update_option( 'default_product_manufacture', $make_default_id );
			}
		}
	}
	 ...
	 * @return string
	 */
	public function product_manufacture_column( $columns, $column, $id ) {
		if ( 'thumb' === $column ) {
	...
	public function disable_checked_ontop( $args ) {
		if ( ! empty( $args['taxonomy'] ) && 'product_manufacture' === $args['taxonomy'] ) {
	 ...
	 * @return void
	 */
	public function scripts_at_product_manufacture_screen_footer() {
		if ( ! isset( $_GET['taxonomy'] ) || 'product_manufacture' !== $_GET['taxonomy'] ) { // WPCS: CSRF ok, input var ok.
			return;
		}
		...
		wc_enqueue_js(
			"(function( $ ) {
				'use strict';
				var product_manufacture = $( 'tr#tag-" . absint( $this->default_cat_id ) . "' );
				product_manufacture.find( 'th' ).empty();
				product_manufacture.find( 'td.thumb span' ).detach( 'span' ).appendTo( product_manufacture.find( 'th' ) );
			})( jQuery );"
		);
	}
}
